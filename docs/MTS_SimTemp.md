...
| **MTS-FR-09-01**  | FR-09          | Verify `stats` Attribute                    | 1. Load driver, read `stats`. <br> 2. Generate samples, trigger an alert, and intentionally overflow the buffer. <br> 3. Read `stats` again. | Initial counters should be zero. `samples` must increase with activity, `alerts` increment when the threshold is crossed, `overruns` increments on buffer overwrite, and the `alert_pending`/`overflow_pending` fields should reflect the current sticky state. Attribute remains read-only. | System    | High     |
